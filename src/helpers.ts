...
export function transformTweetData(input: any): Tweet | null {
  ...
  // Extracting all image URLs if available
  ...
  if (media && media.length > 0) {
    media.forEach((item: any) => {
      if (item.media_url_https) {
        image_urls.push(item.media_url_https);
      }
    });
  }

  const transformed: Tweet & { images: string[] } = {
    ...
    images: image_urls,
    mediaDetails: tweet.legacy.extended_entities?.media,
  };
